...
void send_with_kill(pid_t catcher_pid, int signals_count, sigset_t* mask) {
...
void send_with_sigqueue(pid_t catcher_pid, int signals_count, sigset_t* mask) {
...
void send_with_sigrt(pid_t catcher_pid, int signals_count, sigset_t* mask) {
    for (int i = 0; i < signals_count; ++i) {
...
int main(int argc, char** argv) {
    ...
    if (strcmp(mode, "KILL") == 0) {
        send_with_kill(catcher_pid, signals_count, &mask);
    } else if (strcmp(mode, "SIGQUEUE") == 0) {
        send_with_sigqueue(catcher_pid, signals_count, &mask);
    } else if (strcmp(mode, "SIGRT") == 0) {
        send_with_sigrt(catcher_pid, signals_count, &mask);
    } else {
        perror("invalid mode");
        return EXIT_FAILURE;
    }
