...
#define TIME_MEASUREMENTS_FILENAME "pomiar_zad_5lib.txt"
...
char* get_line(char* buffer, int file) {
    ...
    while (buffer[buffer_position - 1] != '\n') {
        ...
        if (buffer_position == 49 && buffer[buffer_position] != '\n') {
            buffer[buffer_position + 1] = '\n';
            buffer[buffer_position + 2] = '\0';
            return buffer;
        }
        buffer_position++;
    }
...
void break_lines(int read_file, int write_file) {
    char buffer[52];
    while (get_line(buffer, read_file) != NULL) {
        write(write_file, buffer, strlen(buffer));
    }
}
...
int main(int argc, char** argv) {
    ...
    int file_b = open(filename_b, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
    ...
    WITH_TIME_MEASURED(break_lines(file_a, file_b), results_file);
    close(file_a);
    close(file_b);
    close(results_file);
}
