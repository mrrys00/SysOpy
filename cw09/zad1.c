...
int wait_cnt = 0, in_office_raindeers = 0, waiting_elfs_pids[3], actors_num = 20;
...
int main()
{
    ...
    for (int i = 0; i < elfs_num; i++)
        pthread_create(&elf_ptr[i], NULL, elf_thread, (void *)&IDs[i]);

    // tworzenie reniferów
    for (int i = 0; i < raindeer_num; i++)
        pthread_create(&raindeer_ptr[i], NULL, raindeer_thread, (void *)&IDs[i]);
